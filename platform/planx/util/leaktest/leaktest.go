...
func Check2(orig map[string]bool, t ErrorReporter) {
	deadline := time.Now().Add(5 * time.Second)
	for {
		var leaked []string
		for _, g := range interestingGoroutines() {
...
func _ps(g string) string {
	sl := strings.SplitN(g, "\n", 2)
	if len(sl) != 2 {
		return g
	}
	sl0 := strings.Split(sl[0], " ")
	if len(sl0) >= 2 && sl0[0] == "goroutine" {
		if _, err := strconv.Atoi(sl0[1]); err == nil {
			sl[0] = fmt.Sprintf("%s %s", "goroutine", sl0[1])
