...
AtcClientMontifyModal  = React.createClass
  getInitialState: () ->
        {
            data : @props.data
        }
  ...
  montify: () ->
    profile_data = @refs.selector.GetSelectedProfile()
...
AtcClientNewModal  = React.createClass
  ...
  set_can_create: (nip) ->
        @setState {
          ip : nip
          is_can_create: @state.is_can_create & true
        }
...
AtcClientMontify = React.createClass
  getInitialState: () ->
        set = new AtcSettings()
        return {
  ...
  changeSetting: (setting) ->
      @setState {
        settings : @state.client.mkSetting(setting)
      }
      @state.client.shape nullfunc, {down: @state.settings.down, up: @state.settings.up}
      @setState {
        is_turn_on : '1'
        is_waiting : false
      }
  ...
  render:() ->
      ...
      <ButtonToolbar>
        <ModalTrigger modal={
          <AtcClientMontifyModal
            data        = { @props.data }
            setCb       = @changeSetting
            currSetting = { @state.settings }
            name = "Atc Montify"/>}>
            <Button bsStyle='danger'>Montify</Button>
        </ModalTrigger>
        <Button
...
AtcClientList = React.createClass
  ...
  AddOneToTable : ( data ) ->
    <tr key={data.ip} >
        <td>{data.ip}</td>
        <td><AtcClientMontify data = {data}/></td>
    </tr>
