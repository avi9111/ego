...
def get_s3_list_log(daterange, prefixpath):
    prefix = prefixpath
    total_size = 0
    REGION = "cn-north-1"
    conn = s3.connect_to_region(REGION)
...
def get_file_list(daterange, prefixpath):
    if islocal:
        ...
        return get_s3_list_log(daterange, prefixpath)
...
if __name__ == '__main__':
    ...
    for i in range((end - begin).days + 1):
        ...
        if len(ChannelList) > 0:
            ...
            union_df = df_temp_uid_temp.union(uidlogNoChannel_df_temp)
            final_uidChannel_Result = union_df.rdd.map(lambda x: (x.uid, x)).reduceByKey(
                min_time).values()
            ...
            upload(uidlogByChannel_df.toPandas(), 'cn-north-1', "prodlog", '/%s/uid_ByChannelcreatTime.json' % target_path)
        print time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(time.time()))), "union today uid_ByChannelcreatTime "

        # 处理支付log

        historyPayLogFile = get_s3_tempdata("%s_payLog" % day)
        historyPayLogData = spark.read.json(historyPayLogFile)
        historyPayLogData_pd = historyPayLogData.toPandas()
        paylog_data_pd = paylog_data.toPandas()

        payLog_result = pd.concat([historyPayLogData_pd,paylog_data_pd])
        print time.strftime("%Y-%m-%d %H:%M:%S",
                            time.localtime(int(time.time()))), "concat today Paylog "
        upload(payLog_result, 'cn-north-1', "prodlog", '/%s_payLog.json' % (targetpay_path + day))
