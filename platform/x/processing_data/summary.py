...
if __name__ == '__main__':
    ...
    for i in range((end - begin).days + 1):
        ...
        new_channel_name = new_channelUid.withColumnRenamed("count", "newPlayer")
        # 将渠道号 -->Quick渠道号
        ...
        sid_join = login_sid.join(new_acid_name, "sid", 'outer').join(payBySid, "sid", 'outer')

        ch_join = login_ch.join(new_channel_name, "ch", 'outer').join(payByChannel, "ch", 'outer')
        # 开始计算 付费率，arpu，arrpu
        # 分服务器
        aa = sid_join.withColumn("payPercent", sid_join.PayPeoples / sid_join.loginPlayer)
        bb = aa.withColumn("ARPU", aa.sumpay / aa.loginPlayer)
        summaryBySid = bb.withColumn("ARPPU", aa.sumpay / aa.PayPeoples)
        # 分大区
        aa = gid_join.withColumn("payPercent", gid_join.PayPeoples / gid_join.loginPlayer)
        bb = aa.withColumn("ARPU", aa.sumpay / aa.loginPlayer)
        summaryByGid = bb.withColumn("ARPPU", aa.sumpay / aa.PayPeoples)
        # 分渠道
        aa = ch_join.withColumn("payPercent", ch_join.PayPeoples / ch_join.loginPlayer)
        bb = aa.withColumn("ARPU", aa.sumpay / aa.loginPlayer)
        summaryByCh = bb.withColumn("ARPPU", aa.sumpay / aa.PayPeoples)