...
def batch_write(conn, sleep_interval, table_name, put_requests):
    ...
    while True:
        ...
        if len(unprocessed_items) > 0 and i <= MAX_RETRY:
            logging.warn(str(len(unprocessed_items)) + " unprocessed items, retrying.. [" + str(i) + "]")
            request_items = unprocessed_items
            i += 1
            time.sleep(i)
        else:
            logging.warn("Max retries reached, failed to processed batch write: " + json.dumps(unprocessed_items,
                                                                                               indent=JSON_INDENT))
            logging.warn("Ignoring and continuing..")
            break
...
def do_restore(conn, sleep_interval, source_table, destination_table, write_capacity):
    ...
    if not args.schemaOnly:
        ...
        data_file_list.sort()
        logging.warn("{0}".format(data_file_list))
        for data_file in data_file_list:
            ...
            logging.warn("Count {0}, {1}, {2}".format(item_data['Count'], item_data['ScannedCount'], len(item_data['Items'])))
